...
const getTextBetweenParentheses = (editor: vscode.TextEditor) => {
  const acvite = editor.selection.active;
...
const findFirstLineIndexOfBlock = (
  ...
  if (document.lineAt(beforeLineIndex - 1).isEmptyOrWhitespace) {
    return beforeLineIndex;
  }
...
const getBlockToRun = (editor: vscode.TextEditor) => {
  ...
  const startLineIndex = findFirstLineIndexOfBlock(
    editor.document,
    cursorLineIndex
  );
  const lines = [];
    ...
    () => {
      ...
      if (!editor || !terminal) {
        vscode.window.showInformationMessage(
          "Need to have an active editor and an active terminal"
        );
        return;
      }
